...
Empfohlene Reihenfolge: 1 → 2 → 2b → 3 → 4 → 5 → 6 → 7 → 8. Phasen 1+2 liefern den größten sichtbaren Paritätssprung (Stand 2026-07-09: **alle Phasen 1–8 erledigt ✅** — Plan komplett). Jede Phase = eine Opus/Sonnet-Session.
...
> **Status: ✅ fertig (2026-07-09, uncommittet).** Beide Features wie geplant, letzte Phase. **Neu:** `src/lib/analysis/puzzles.ts` (+`.test.ts`, 9 Tests) — `extractPuzzles(moves, fens, evalResults, moveAnalyses, opts?)` → `Puzzle{moveIndex, fenBefore, bestSan, playedSan, classification}[]`; filtert Mistake/Miss/Blunder-Plies (`PUZZLE_CLASSES`) mit **eindeutig bester Antwort** (Uniqueness-Gate `|cp − secondBestCp| ≥ PUZZLE_GAP_MIN=100`, White-absolut aber magnitudenbasiert → auch Schwarz-am-Zug korrekt), skippt fehlende Engine-Referenz / best==played; pure, kein Engine-Call zur Lösezeit (Lösung steht aus der Batch-Analyse fest). `src/hooks/usePlayout.ts` — Practice-vs-Engine-State: `start(fen)` setzt userColor = Seite-am-Zug (User zieht zuerst, Engine nie am Anfang), Reply-Effekt feuert `requestPlayMove` sobald Engine am Zug; `genRef` verwirft in-flight-Replies bei Exit/Restart; `engineThinking` **abgeleitet** (nicht State) = `active && !gameOver && !isUserTurn` — vermeidet synchrones setState-im-Effekt (Lint). `src/hooks/usePuzzles.ts` — sequenzieller Drill (`start/tryMove/retry/reveal/next/exit`), `tryMove` prüft SAN gegen `bestSan` (exakt, wie Retry), `attempt.fen` fürs Board. `src/components/PlayoutPanel.tsx` (Status „Your move"/„Engine is thinking…"/Matt/Remis + Zugzähler + Exit) und `PuzzlePanel.tsx` (Fortschritt n/total, Seite-am-Zug, Klassifikation, ✓/✗-Feedback, Try again/Show solution/Next/Finish, Abschluss-Score-Screen). **Engine-Eingriff (`useEngine.ts`):** neuer `requestPlayMove(fen, movetimeMs=500): Promise<uci|null>` — zweiter Request-Pfad neben Batch-Analyse, gleicher Worker: postet `position fen`+`go movetime`, der `bestmove`-Handler routet zuerst auf `playRequestRef` (resolve + `return`, **kein** evalResults/result-Mutieren) bevor die Queue/Result-Logik greift; eigener Watchdog (movetime+3 s, force `stop`+null-resolve); guardt gegen `!isReady`/laufende Analyse. **`App.tsx`:** `usePlayout`/`usePuzzles` instanziiert, `puzzleList` memoized, Board-Übernahme-Block (playout/puzzle > retry > review-Submodi) überschreibt `viewFen`/`viewFrom/To`/`viewArrow`/`viewEval`/`boardInteractive`/`boardOnDrop`/`boardResultBadge` und unterdrückt Coach/Threat/Engine-Line-Overlays (`boardExtraArrows`/`boardExtraSquares=undefined`); Sidebar rendert PlayoutPanel/PuzzlePanel vor den Kapitel-Zweigen; Tastatur-Nav während Practice/Puzzle deaktiviert und **die NavControls-Buttons (⏮◀▶⏭/🔗/📷) via neuem `takeover`-Prop ebenfalls** (sonst würden sie den darunterliegenden `currentPly` still verschieben und die Zustands-Wiederherstellung beim Exit untergraben; Flip/Theme/Settings bleiben aktiv); „Practice from here"-Button in `ReviewView` (idle, `canPractice=isReady && currentPly>0`), „Puzzles (n)"-Button in `SummaryView` (nur wenn n>0). **264 Vitest-Tests grün (+9), `tsc -b`+`eslint` sauber.** Live per Preview-MCP (Port 5199) verifiziert an `1. f3 e5 2. g4 d5 3. Nc3 Bc5` (verpasstes Fool's Mate): Analyze → Summary zeigt „Puzzles (2)" → Puzzle 1/2 („Black to move", „Blunder", Eval-Bar −M1) → Show solution zeigt „Qh4#" + grünen d8→h4-Pfeil + Stellung nach Matt → Next → Puzzle 2/2 → Finish → Score-Screen „0/2" → Back to review; Start Review → „Practice from here" öffnet PlayoutPanel („You are playing Black", „Your move"), Exit Practice stellt Review-Ansicht wieder her; keine Konsolenfehler. Der `go movetime`→`bestmove`-Mechanismus zusätzlich mit eigenständigem Worker gegen dieselbe Teststellung bestätigt (`bestmove d8h4`). ⚠️ Die eigentliche @dnd-kit-Drag-Geste ließ sich im automatisierten Preview nicht auslösen (Harness-Limitierung, kein Code-Bug) — die User-Zug-Plumbing ist identisch zum in früheren Phasen verifizierten Retry-Feature. **Plan damit komplett (Phasen 1–8 alle ✅).**
